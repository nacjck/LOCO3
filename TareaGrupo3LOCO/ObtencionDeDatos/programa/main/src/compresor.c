...
DtCompresion crearDtCompresion( FILE * archivoOriginal ) {
    ...
    //res.tasaCompFila = malloc(anchoImagen * sizeof(int));
    //memset(res.tasaCompFila, 0, anchoImagen);
    res.tasaCompTotal = 0;
...
void actualizarDatosCompresion( DtCompresion * dtComp, int cantidadComprimida ) {
    ...
    /*cantidadComprimidaFilaActual += cantidadComprimida;
    if (esFinDeLinea()) {
        dtComp->tasaCompFila[filaActual] = cantidadComprimidaFilaActual;
        cantidadComprimidaFilaActual = 0;
        filaActual++;
    }*/
