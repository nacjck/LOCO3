...
int main( int argc, char* argv[] ) {
    ...
    puts("\nIniciando programa...");
    DtCompresion dtComp = comprimir(parametros.archivoEntrada,parametros.archivoSalida,parametros.s,parametros.modalidad);
    puts("\nCompresion finalizada.");
    puts("\nResumen:");
    printf("Cantidad de pixeles en archivo original: %ld\n",dtComp.cantPixeles);
    printf("Cantidad de bytes en archivo comprimido: %ld\n",dtComp.cantComprimida >> 3);
    printf("Tasa de compresion: %f\n",dtComp.tasaCompTotal);
    puts("");
